...
Adobe ships this fix as a `vendor/`-level hotfix rather than as a tagged Magento release, so there is no new upstream version to move to — 3.5.0 tracks Magento Open Source 2.4.9, the same as 3.4.0. We have mapped the patch onto the Mage-OS source tree so you get it through a normal Composer upgrade.

#### StyleSmuggler — CVE-2026-75650

The exploit chain is two-stage: an attacker first plants PHP into a file the application will later read — typically a failure report — then triggers evaluation of that content through the transactional email template pipeline. No admin session and no user interaction are required.
...
2. **Scan for implants.** [Sansec's StyleSmuggler write-up](https://sansec.io/research/stylesmuggler-0day) documents the observed payloads and what to look for — start there. Check for unexpected files under `pub/` and `var/report/`, and for modifications in `app/code/`. Assume there may be more than one backdoor.
...
Step 3 is the one most likely to be skipped, and the one that matters most.
...
The certified stack is unchanged from 3.4.0: PHP 8.4, Composer 2.10.2, MySQL 8.4, OpenSearch 3, RabbitMQ 4.1, Valkey 8, Varnish 7.7 and nginx 1.28.
...
[@rhoerr](https://github.com/rhoerr), [@TuVanDev](https://github.com/TuVanDev), [@emetik](https://github.com/emetik), [@ProxiBlue](https://github.com/ProxiBlue), [@marcelmtz](https://github.com/marcelmtz)
